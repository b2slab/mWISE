...
performanceEvaluation <- function(Annotated.dataset, df.Ref, top.cmps, do.Par = TRUE, nClust){
  Peak.Cpd <- Annotated.dataset$Annotated.Tab
  merge.df <- merge(df.Ref[,c("Peak.Id", "Kegg")],
                    Peak.Cpd, by = "Peak.Id")
  stages <- c("Matching", "Filtering", "Diffusion")
  Annotated.dataset$Ranked.Tab <- Annotated.dataset$Ranked.Tab[Annotated.dataset$Ranked.Tab$Ranking<=top.cmps,]
  metrics.stages <- plyr::ldply(1:3, function(j){
    i <- c(1,3,5)[j]
    df <- Annotated.dataset[[i]]
    ...
    Metrics <- plyr::ddply(merge.df, "Peak.Id", function(dy){
      ...
      if (nrow(dx)>0){
        TP <- sum(dx$Compound %in% Ref.Cmp)
        TP <- ifelse(TP>0, 1, 0)
        FP <- sum(!(dx$Compound %in% Ref.Cmp))
        FN <- sum(!(Ref.Cmp %in% dx$Compound))
        TN <- sum((!(dy$Compound %in% dx$Compound)) & (!(dy$Compound %in% Ref.Cmp)))
        data.frame(TP = TP, FP = FP, TN = TN, FN = FN, Background = sum(TP, FP, TN, FN))
      } else {
        TP <- 0
        FP <- 0
        FN <- 1
    ...
    Total.Metrics <- as.data.frame(t(colSums(Metrics[,c("TP", "FP", "TN", "FN")])))
    ...
    Total.Metrics$F1 <- round(Total.Metrics$TP/(Total.Metrics$TP+(0.5*(Total.Metrics$FP+Total.Metrics$FN))),2)
