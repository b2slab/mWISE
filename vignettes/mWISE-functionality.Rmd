...
Below, the table used to perform the maching is loaded and the first rows are shown. The columns contain the KEGG identifiers, their exact masses, different adducts and fragments, the acquisition mode polarity of each adduct or fragment and the mass-to-charge ratio expected. 
...
Below, an example where only quasi-molecular adducts are considered is shown. 

```{r }
Info.Add.subset <- Info.Add[Info.Add$quasi==1,]
Info.Add.subset
```

```{r }
Cpd.Add.built <- CpdaddPreparation(Info.Add = Info.Add.subset, do.Par = TRUE, nClust = 2)
head(Cpd.Add.built)
```
...
Peak.List <- Trypanosoma.dataset$Negative$Input
...
```{r}
MH.Tab <- recoveringPeaks(df = Annotated.Tab, 
                          polarity = "negative")
...
Annotated.List <- mWISE.annotation(Peak.List = Peak.List, 
                                   polarity = "negative", Cpd.Add = Cpd.Add,
                                   #Add.List = selectedAdds,
                                   diffusion.input.type = "probability", 
                                   score = "z", Add.Id = c("M-H", "M+Cl"),
                                   Unique.Annotation = FALSE,
                                   graph.name = "fella", 
                                   Intensity.idx = Intensity.idx, 
                                   do.Par = TRUE, nClust = 2)
...
```{r}
performanceEvaluation(Annotated.dataset = Annotated.List, df.Ref = df.Ref, top.cmps = 3)
```
